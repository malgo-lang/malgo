; ModuleID = 'test/testcases/malgo/NestedMatch.mlg'
source_filename = "test/testcases/malgo/NestedMatch.mlg"

@str80 = unnamed_addr constant [3 x i8] c"OK\00"

declare void @GC_init()

declare ptr @malgo_print_string(ptr)

declare ptr @malgo_exit_failure(ptr)

declare ptr @malgo_malloc(i64)

define internal ptr @_M5False48test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 0, ptr %3, align 1
  ret ptr %2
}

define internal ptr @_M6False148test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 0, ptr %3, align 1
  ret ptr %2
}

define internal ptr @_M6False248test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 0, ptr %3, align 1
  ret ptr %2
}

define internal ptr @_M4True48test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 1, ptr %3, align 1
  ret ptr %2
}

define internal ptr @_M5True148test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 1, ptr %3, align 1
  ret ptr %2
}

define internal ptr @_M5True248test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 1, ptr %3, align 1
  ret ptr %2
}

define internal ptr @_M24malgo_x5Fprint_x5Fstring48test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0, ptr %_M7p_x242448test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8Temporal_0) {
  %2 = call ptr @malgo_print_string(ptr %_M7p_x242448test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8Temporal_0)
  ret ptr %2
}

define internal ptr @_M4main48test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0, ptr %_M20_x24_x5F_x241d_x242648test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8Temporal_0) {
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 1, ptr %3, align 1
  %4 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  %5 = load i8, ptr %4, align 1
  switch i8 %5, label %switch_default_6 [
    i8 0, label %switch_branch_False2_0
    i8 1, label %switch_branch_True2_0
  ]

switch_branch_False2_0:                           ; preds = %1
  %6 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %7 = getelementptr { i8, {} }, ptr %6, i32 0, i32 0
  store i8 1, ptr %7, align 1
  %8 = getelementptr { i8, {} }, ptr %6, i32 0, i32 0
  %9 = load i8, ptr %8, align 1
  switch i8 %9, label %switch_default_2 [
    i8 0, label %switch_branch_False1_0
    i8 1, label %switch_branch_True1_0
  ]

switch_branch_False1_0:                           ; preds = %switch_branch_False2_0
  %10 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %11 = getelementptr { i8, {} }, ptr %10, i32 0, i32 0
  store i8 1, ptr %11, align 1
  %12 = getelementptr { i8, {} }, ptr %10, i32 0, i32 0
  %13 = load i8, ptr %12, align 1
  switch i8 %13, label %switch_default_0 [
    i8 0, label %switch_branch_False_0
    i8 1, label %switch_branch_True_0
  ]

switch_branch_False_0:                            ; preds = %switch_branch_False1_0
  %14 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %15 = getelementptr { i8, {} }, ptr %14, i32 0, i32 0
  store i8 0, ptr %15, align 1
  %16 = call ptr @malgo_exit_failure(ptr %14)
  ret ptr %16

switch_branch_True_0:                             ; preds = %switch_branch_False1_0
  %17 = call ptr @malgo_print_string(ptr @str80)
  ret ptr %17

switch_default_0:                                 ; preds = %switch_branch_False1_0
  unreachable

switch_branch_True1_0:                            ; preds = %switch_branch_False2_0
  %18 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %19 = getelementptr { i8, {} }, ptr %18, i32 0, i32 0
  store i8 0, ptr %19, align 1
  %20 = getelementptr { i8, {} }, ptr %18, i32 0, i32 0
  %21 = load i8, ptr %20, align 1
  switch i8 %21, label %switch_default_1 [
    i8 0, label %switch_branch_False_1
    i8 1, label %switch_branch_True_1
  ]

switch_branch_False_1:                            ; preds = %switch_branch_True1_0
  %22 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %23 = getelementptr { i8, {} }, ptr %22, i32 0, i32 0
  store i8 0, ptr %23, align 1
  %24 = call ptr @malgo_exit_failure(ptr %22)
  ret ptr %24

switch_branch_True_1:                             ; preds = %switch_branch_True1_0
  %25 = call ptr @malgo_print_string(ptr @str80)
  ret ptr %25

switch_default_1:                                 ; preds = %switch_branch_True1_0
  unreachable

switch_default_2:                                 ; preds = %switch_branch_False2_0
  unreachable

switch_branch_True2_0:                            ; preds = %1
  %26 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %27 = getelementptr { i8, {} }, ptr %26, i32 0, i32 0
  store i8 0, ptr %27, align 1
  %28 = getelementptr { i8, {} }, ptr %26, i32 0, i32 0
  %29 = load i8, ptr %28, align 1
  switch i8 %29, label %switch_default_5 [
    i8 0, label %switch_branch_False1_1
    i8 1, label %switch_branch_True1_1
  ]

switch_branch_False1_1:                           ; preds = %switch_branch_True2_0
  %30 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %31 = getelementptr { i8, {} }, ptr %30, i32 0, i32 0
  store i8 1, ptr %31, align 1
  %32 = getelementptr { i8, {} }, ptr %30, i32 0, i32 0
  %33 = load i8, ptr %32, align 1
  switch i8 %33, label %switch_default_3 [
    i8 0, label %switch_branch_False_2
    i8 1, label %switch_branch_True_2
  ]

switch_branch_False_2:                            ; preds = %switch_branch_False1_1
  %34 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %35 = getelementptr { i8, {} }, ptr %34, i32 0, i32 0
  store i8 0, ptr %35, align 1
  %36 = call ptr @malgo_exit_failure(ptr %34)
  ret ptr %36

switch_branch_True_2:                             ; preds = %switch_branch_False1_1
  %37 = call ptr @malgo_print_string(ptr @str80)
  ret ptr %37

switch_default_3:                                 ; preds = %switch_branch_False1_1
  unreachable

switch_branch_True1_1:                            ; preds = %switch_branch_True2_0
  %38 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %39 = getelementptr { i8, {} }, ptr %38, i32 0, i32 0
  store i8 0, ptr %39, align 1
  %40 = getelementptr { i8, {} }, ptr %38, i32 0, i32 0
  %41 = load i8, ptr %40, align 1
  switch i8 %41, label %switch_default_4 [
    i8 0, label %switch_branch_False_3
    i8 1, label %switch_branch_True_3
  ]

switch_branch_False_3:                            ; preds = %switch_branch_True1_1
  %42 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %43 = getelementptr { i8, {} }, ptr %42, i32 0, i32 0
  store i8 0, ptr %43, align 1
  %44 = call ptr @malgo_exit_failure(ptr %42)
  ret ptr %44

switch_branch_True_3:                             ; preds = %switch_branch_True1_1
  %45 = call ptr @malgo_print_string(ptr @str80)
  ret ptr %45

switch_default_4:                                 ; preds = %switch_branch_True1_1
  unreachable

switch_default_5:                                 ; preds = %switch_branch_True2_0
  unreachable

switch_default_6:                                 ; preds = %1
  unreachable
}

define internal ptr @_M24malgo_x5Fexit_x5Ffailure48test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr %0, ptr %_M7p_x242548test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8Temporal_0) {
  %2 = call ptr @malgo_exit_failure(ptr %_M7p_x242548test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8Temporal_0)
  ret ptr %2
}

define i32 @main(ptr %0) {
  call void @GC_init()
  call void @"malgo_load_test/testcases/malgo/NestedMatch.mlg"()
  %2 = call ptr @malgo_malloc(i64 ptrtoint (ptr getelementptr inbounds ({ i8, {} }, ptr null, i32 1) to i64))
  %3 = getelementptr { i8, {} }, ptr %2, i32 0, i32 0
  store i8 0, ptr %3, align 1
  %4 = call ptr @_M4main48test_x2Ftestcases_x2Fmalgo_x2FNestedMatch_x2Emlg8External(ptr null, ptr %2)
  ret i32 0
}

define internal void @"malgo_load_test/testcases/malgo/NestedMatch.mlg"() {
  ret void
}
