...
compile :: Monad m => Syntax.Expr Name -> StateT Int m (MIR.Program TypeCheck.TypedID)
compile ast = do
    renamed <-
        doMalgoT (Rename.rename ast) >>= \case
            Left x -> error $ show x
            Right x -> return x
    typechecked <-
        doMalgoT (TypeCheck.typeCheck renamed) >>= \case
            Left x -> error $ show x
            Right x -> return x
    knormal <-
        doMalgoT (KNormal.knormal typechecked) >>= \case
            Left x -> error $ show x
            Right x -> return x
    beta <-
        doMalgoT (Beta.betaTrans knormal) >>= \case
            Left x -> error $ show x
            Right x -> return x
