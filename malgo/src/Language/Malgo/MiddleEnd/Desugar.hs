...
toExp (S.Fn _ ps e) = runDef $ do
  ps' <- traverse ((\p -> newLocalId (p ^. idName) (C.typeOf p)) . fst) ps
  e <- do
...
toExp (S.Let _ (S.FunDec fs) e) = do
  for_ fs $ \(_, f, _, _, _) ->
    (at f ?=) =<< newTopLevelId (f ^. idName) (C.typeOf f)
...
crushPat (S.VarP _ x) = \e -> do
  x' <- (at x <?=) =<< newLocalId (x ^. idName) (C.typeOf $ typeOf x)
  Bind x' <$> e
crushPat (S.TupleP _ xs) = go xs []
  where
    ...
    go (p : ps) acc e = do
      x <- newLocalId "p" (C.typeOf $ typeOf p)
      go ps (x : acc) $ do
        clause <- crushPat p e
