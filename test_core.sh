...
for file in `ls ./testcases | grep mlg`; do
  echo -e "\n=== $file no opt ==="
  stack exec malgo -- --no-opt --core-mode --no-lambdalift ./testcases/$file -o ./tmp/$file.ll && \
...
for file in `ls ./testcases/bug | grep mlg`; do
  echo -e "\n=== $file no opt ==="
  stack exec malgo -- --no-opt --core-mode --no-lambdalift ./testcases/bug/$file -o ./tmp/$file.ll && \
  clang -lgc ./examples/corelib.c ./tmp/$file.ll -o ./tmp/$file.out && \
  ./tmp/$file.out && \
  rm ./tmp/$file.ll && \
