...
import Control.Lens ((.~), (<>~))
import KaKKo.Parser (parseKaKKo)
...
main = do
  ...
  case command of
    ToLL opt -> do
      ...
      let parsedAst = case parseKaKKo (_srcName opt) src of
            Right x -> x
  ...
  command <-
    execParser
      ( info ((subparser toLL <|> subparser lsp) <**> helper) $
          fullDesc
            <> header "ka K ko programming language"
      )
  case command of
    ToLL opt -> do
  ...
  where
    toLL =
      command "to-ll" $
        info (ToLL <$> toLLOpt) $
          fullDesc
            <> progDesc "Compile ka K ko file (.kako) to LLVM Textual IR (.ll)"
            <> header "ka K ko to LLVM Textual IR Compiler"
    lsp =
      command "lsp" $
        info (Lsp <$> lspOpt) $
          fullDesc
            <> progDesc "Language Server for ka K ko"
            <> header "ka K ko Language Server"
