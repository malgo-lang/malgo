...
index :: (Reader ModulePathList :> es, S.State (HashMap ModuleName Index) :> es, IOE :> es) => TcEnv -> Module (Malgo 'Refine) -> Eff es Index
index tcEnv mod = do
...
indexModule :: (State IndexEnv :> es, Reader ModulePathList :> es, S.State (HashMap ModuleName Index) :> es, IOE :> es) => Module (Malgo 'Refine) -> Eff es ()
