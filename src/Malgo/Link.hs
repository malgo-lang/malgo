...
link :: (MonadIO m, MonadReader env m, HasField "_modulePaths" env [FilePath], Binary a) => Interface -> Program a -> m (Program a)
...
loadCore :: (MonadReader s m, MonadIO m, HasField "_modulePaths" s [FilePath], Binary a) => ModuleName -> m (Program a)
