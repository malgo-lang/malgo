...
optimizeProgram Program {..} = do
  state <-
    {-# SCC "buildState" #-}
      execState mempty do
        {-# SCC "checkInlinable_functions" #-} for_ functions $ \(name, ps, t, e) -> checkInlinable $ LocalDef name t (Fun ps e)
        {-# SCC "checkInlinable_variables" #-}
          for_ variables $ \case
            (name, t, Let [LocalDef f _ (Fun ps e)] (Atom (Var v))) | f == v -> checkInlinable $ LocalDef name t (Fun ps e)
            _ -> pass
  variables <- {-# SCC "optimizeExpr_variables" #-} traverse (\(n, t, e) -> (n,t,) <$> optimizeExpr state e) variables
  functions <- {-# SCC "optimizeExpr_functions" #-} traverse (\(n, ps, t, e) -> (n,ps,t,) <$> optimizeExpr state e) functions
  ...
  let roots = {-# SCC "roots" #-} filter (\x -> x.id.sort `elem` [External, Native] && x.id.moduleName == moduleName) $ map (view _1) functions <> map (view _1) variables
  ...
  variables <- {-# SCC "filter_variables" #-} pure $ filter (\(n, _, _) -> isUsed n) variables
  functions <- {-# SCC "filter_functions" #-} pure $ filter (\(n, _, _, _) -> isUsed n) functions
