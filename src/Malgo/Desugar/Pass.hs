...
-- | MalgoからCoreへの変換
desugar ::
  (IOE :> es, State (Map ModuleName Interface) :> es, State Uniq :> es, Workspace :> es) =>
  TcEnv ->
  Module (Malgo 'Refine) ->
...
dsBindGroup :: (State (Map ModuleName Interface) :> es, State DsState :> es, IOE :> es, Reader ModuleName :> es, State Uniq :> es, Workspace :> es) => BindGroup (Malgo Refine) -> Eff es [Def]
dsBindGroup bg = do
...
dsExpr (G.Record (Typed (GT.TyRecord recordType) _) kvs) = runDef $ do
  ...
  v <- withMeta (RecordT (Map.fromList kts)) <$> newTemporalId "record"
  pure $ C.Let [C.LocalDef v (C.typeOf v) (C.Record $ Map.fromList kvs')] $ Atom $ C.Var v
