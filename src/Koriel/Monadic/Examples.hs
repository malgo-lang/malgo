...
prelude = program do
  ...
  printStringId <- newExternalId "printString" (TPtr (TNode 0 [TPtr TAny, TFun [TPtr TAny, TString] (TNode 0 [])])) (ModuleName "Prelude")
  preDef printStringId
  defVar printStringId =<< exp do
    arg0 <- newInternalId "arg0" TString
    var <-
      closure . Func [arg0] =<< exp do
        Unit . Var <$> apply print_string_id [Var arg0]
    pure $ Unit $ Var var
