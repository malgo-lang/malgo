...
import           LLVM.AST.Type               as L
import           Relude                      hiding (Type)
import           Relude.Extra.Map            hiding (size)

data TransToLIR

instance Pass TransToLIR (M.Program M.Type (ID M.Type)) (L.Program L.Type (ID L.Type)) where
...
type GenExpr a = StateT [(ID L.Type, L.Expr L.Type (ID L.Type))] (ReaderT Env MalgoM) a
...
push :: MonadState [(ID L.Type, L.Expr L.Type (ID L.Type))] m => ID L.Type -> L.Expr L.Type (ID L.Type) -> m ()
...
alloca ty size = do
  ...
  push i $ Alloca (L.ptr $ convertType ty) size
  pure i
