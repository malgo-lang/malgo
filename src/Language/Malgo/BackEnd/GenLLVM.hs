...
genInst (IR.Undef ty) =
  pure $ ConstantOperand $ C.Undef (convertType ty)
...
unit = flip bitcast (ptr $ StructureType False []) =<< mallocBytes (sizeof (StructureType False [])) Nothing
...
genConstant (String bytes) = do
  n <- fresh
  p <- global n (ArrayType (toEnum $ length bytes + 1) i8) (Array i8 $ map (Int 8 . toInteger) (bytes <> [0]))
  bitcast p (ptr i8)
