...
renameTopLevel xs (ScAnn ss x _ : ds)
  | x `elem` map (view _1) xs
  = renameError ss $ pPrint x <+> "is already defined."
  | otherwise
  = do
    x' <- newId x
    renameTopLevel ((x, x') : xs) ds
...
renameDecl (ScDef ss x ps e) = do
  x'  <- lookupName ss x
  ps' <- mapM newId ps
  local (Map.fromList (zip ps ps') <>) $ ScDef ss x' ps' <$> renameExpr e
...
renameDecl (TypeDef ss x ps t) = do
  x'  <- lookupName ss x
  ps' <- mapM newId ps
  local (Map.fromList (zip ps ps') <>) $ TypeDef ss x' ps' <$> renameType t
...
renameExpr (BinOp ss op e1 e2) =
  BinOp ss op <$> renameExpr e1 <*> renameExpr e2
renameExpr (If ss c t f) =
  If ss <$> renameExpr c <*> renameExpr t <*> renameExpr f
...
renameTypeScheme (Forall xs t) = do
  xs' <- mapM newId xs
  local (Map.fromList (zip xs xs') <>) $ Forall xs' <$> renameType t
...
renameType (TyMeta _) =
  renameError noSrcSpan "unreachable(renameType (TyMeta _))"
