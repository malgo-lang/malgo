...
initModule :: BS.ShortByteString -> AST.Module
initModule label = AST.defaultModule
  { AST.moduleName = label
  , AST.moduleDefinitions = [ AST.TypeDefinition
                              (Name.Name "unit")
                              (Just $ Type.StructureType False [])
                            ]
  }
...
defineType :: BS.ShortByteString -> Type.Type -> LLVM ()
defineType name typ = addDefn $
  AST.TypeDefinition (Name.Name name) (Just typ)
...
unitTy :: Type.Type
unitTy = Type.NamedTypeReference (Name.Name "unit")

funTy :: Type.Type -> [Type.Type] -> Type.Type
funTy retTy argTys = Type.FunctionType retTy argTys False

voidPtrTy = Type.PointerType (Type.IntegerType 8) (AddrSpace.AddrSpace 0)
