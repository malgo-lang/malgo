...
parseKaKKo :: String -> Text -> Either (ParseErrorBundle Text Void) (Module (Malgo 'Parse))
parseKaKKo = parse do
  sc
  mod <- pModule
  eof
...
pImport = label "import" do
  ...
  void $ pKeyword "rt"
  pure $ Import (Range start end) modName All
...
pScDef =
  label "toplevel function definition" do
    void $ pKeyword "de"
    start <- getSourcePos
    name <- lowerIdent <|> between (symbol "ka") (symbol "ko") operator
    end <- getSourcePos
    e <- pExp
    void $ pKeyword "ne"
...
pClauses = do
  _ <- optional (pOperator "or")
  pClause `sepBy1` pOperator "or"
...
pSinglePat =
  pVarP
    ...
    <|> pRecordP
    <|> pListP
    <|> between (symbol "ka") (symbol "ko") pPat
  where
    pVarP = do
      start <- getSourcePos
      name <- lowerIdent
    ...
    pTupleP = do
      start <- getSourcePos
      ps <- between (symbol "ka") (symbol "ko") do
        p <- pPat
        void $ pOperator ","
...
pTuple = label "tuple" do
  start <- getSourcePos
  xs <- between (symbol "ka") (symbol "ko") do
    x <- pExp
    void $ pOperator ","
...
pSeq = do
  start <- getSourcePos
  stmts <- between (symbol "ka") (symbol "ko") pStmts
  end <- getSourcePos
  pure $ Seq (Range start end) stmts
...
pSingleExp' =
  ...
  where
    ...
    pParens = do
      start <- getSourcePos
      e <- between (symbol "ka") (symbol "ko") pExp
      end <- getSourcePos
...
pSingleType =
  pTyVar
    ...
    <|> try pTyRecord
    <|> pTyBlock
    <|> between (symbol "ka") (symbol "ko") pType
