-- | KaKKo.Driver is the entry point of `kakko to-ll`.
module KaKKo.Driver (compile, compileFromAST, withDump) where
...
compile opt = do
  ...
  parsedAst <- case parseKaKKo (view srcName opt) src of
    Right x -> pure x
