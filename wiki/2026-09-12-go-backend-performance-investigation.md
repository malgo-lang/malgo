...
| | Go | Zig | Chez（全体） | Chez（実行のみ） |
|---|---|---|---|---|
| selfhost Level 1 | **0.28s** | 0.24s | 0.70s | 0.10s |
| fib-deep | **0.29s** | 0.32s | 0.19s | 0.04s |
| 例題（1本あたり） | **0.01s** | 0.01s | 0.16s | ほぼ 0 |

Go は fib-deep で Zig を上回り、Level 1 で Zig の 1.2 倍差まで詰めた。Chez に対しては、起動から終了までなら長時間の純粋計算を除いて優位に立つ。生成コードの実行速度そのものでは届かず、その理由は Go に末尾呼び出しが無いことに尽きる。
